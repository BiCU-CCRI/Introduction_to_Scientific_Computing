...
Since this session is mostly about learning how to work with SLURM, you will use the simulated SLURM environment "LearnSlurm".  
...
3. Let's try to request more resources than are available/allowed. Try to request 500G with `salloc`:
...
2. Use the `scontrol show job <job_id>` command to show detailed information about your job. You can find the job ID from the output of the `squeue` command.  
